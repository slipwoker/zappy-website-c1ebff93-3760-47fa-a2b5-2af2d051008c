    ...
    contactForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
          ...
          body: JSON.stringify({
            websiteId: 'c1ebff93-3760-47fa-a2b5-2af2d051008c',
            name: data.name || '',
            email: data.email || '',
        ...
        if (response.ok) {
          ...
          console.log('⚠️ Zappy: Email API returned non-OK status', response.status, result);
        }
      } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מרכז מונטסורי במושבה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
